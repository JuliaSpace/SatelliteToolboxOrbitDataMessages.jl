...
- `creation_date::Union{NanoDate, Nothing}`: Message creation date (**required**). It can
    be set to `nothing` when reconstructing messages parsed in non-strict mode, but such
    messages cannot be written to a file until a creation date is provided.
