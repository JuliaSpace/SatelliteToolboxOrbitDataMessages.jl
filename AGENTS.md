...
- `ext/SatelliteToolboxTleExt` is a package extension that loads only when `SatelliteToolboxTle` (v2) is loaded; it provides `convert(TLE, omm)`, `OrbitMeanElementsMessage(tle; kwargs...)`, and `convert(OrbitMeanElementsMessage, tle)`.
